(function () {
    ...
    var token = script.getAttribute('data-sitecounter-token');
    var endpoint = script.getAttribute('data-sitecounter-endpoint') || deriveEndpoint(script);

    if (!token || !endpoint) {
        if (window.console && typeof window.console.warn === 'function') {
            window.console.warn('SiteCounter: missing data-sitecounter-token or unable to determine tracking endpoint.');
        }
        return;
    }

    function deriveEndpoint(scriptElement) {
        var scriptSrc = scriptElement.getAttribute('src') || scriptElement.src;
        ...
        try {
            ...
            var basePath = '/';
            var trackerMarkerIndex = path.indexOf('/js/sitecounter-tracker');

            if (trackerMarkerIndex >= 0) {
                basePath = path.slice(0, trackerMarkerIndex + 1);
            } else {
                var jsFolderIndex = path.lastIndexOf('/js/');
                if (jsFolderIndex >= 0) {
                    basePath = path.slice(0, jsFolderIndex + 1);
                }
            }
            ...
            return new URL(basePath + 'track', scriptUrl.origin).toString();
        } catch (e) {
            return '';
        }
    }
    ...
    function onReady() {
        ...
        var hasVisitorCookie = !!getCookie(visitorCookieName);

        if (consentChoice === 'allow') {
            if (!hasVisitorCookie) {
                // If cookies were cleared, ask for consent again before creating a new cookie.
                clearConsentChoice();
            } else {
